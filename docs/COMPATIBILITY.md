...
- The Python engine does not emit a synthetic trace step by default when an `{"advance_time": N}` control token is processed. Use `--emit-time-steps` when timer-driven changes must be visible even without a subsequent external event.
