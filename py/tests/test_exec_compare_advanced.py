...
Falls back to Python engine as reference if SCION is unavailable.
"""
...
def _run_compare(chart: Path, *, generate: bool = True, depth: int = 2, extra: list[str] | None = None, force_python_ref: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    args = [
        ...
        str(chart),
        "--reference",
        (_python_ref() if force_python_ref else _ref_command(root.parents[0])),
    ]
    if generate:
...
def test_parallel_invoke_compare_scion() -> None:
    # Compare Python vs SCION on the parallel complete + finalize chart
    root = Path(__file__).resolve().parents[2]
    chart = root / "tests" / "sweep_corpus" / "parallel_invoke_complete.scxml"
    scion = root / "tools" / "scion-runner" / "scion-trace.cjs"
    if shutil.which("node") is None or not scion.exists():
        pytest.skip("SCION not available")
    env = dict(os.environ)
    env["PYTHONPATH"] = str(root / "py")
    # Preflight SCION on this chart to avoid hard failures
    pre = subprocess.run([
        "node",
        str(scion),
        "-I",
    ...
    if pre.returncode != 0:
        pytest.skip("SCION cannot execute this chart reliably: " + pre.stderr.strip())
    args = [
        sys.executable,
        str(root / "py" / "exec_compare.py"),
        str(chart),
...
def test_finalize_order_compare_scion() -> None:
    # Compare Python vs SCION on finalize ordering chart; guard if SCION can't run it
    root = Path(__file__).resolve().parents[2]
    chart = root / "tests" / "sweep_corpus" / "finalize_order.scxml"
    scion = root / "tools" / "scion-runner" / "scion-trace.cjs"
    if shutil.which("node") is None or not scion.exists():
